...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        "*** YOUR CODE HERE ***"
        if self.exit:
            self.exit.entrance = self  # mutate attribute of existed place
        # END Problem 2
...
class Ant(Insect):
    ...
    def add_to(self, place):
        # Two add_to method implemented in Insect and Ant set different attribute of object Ant and Place mutually
        ...
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if self.can_contain(self.place.ant):
                self.contain_ant(self.place.ant)
            elif self.place.ant.can_contain(self):
                self.place.ant.contain_ant(self)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def helper(place, distance):
            if not place:
                return None
            if (place.bees and not isinstance(place, Hive)) and (
                    self.min_range <= distance <= self.max_range):
...
def rANTdom_else_none(s):
    ...
    assert isinstance(
        s, list
    ), "rANTdom_else_none's argument should be a list but was a %s" % type(
        s).__name__
    if s:
        ...
        if self.place.bees:
            if self.armor <= amount:
                for i in reversed(range(len(self.place.bees))):
                    Insect.reduce_armor(self.place.bees[i],
                                        amount + self.damage)
            else:
                for i in reversed(range(len(self.place.bees))):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.is_first:
            self.is_true_queen = True
            self.is_removeable = False
    ...
    def inspire(self):
        ...
        Once an ant's damage has been doubled, it cannot be doubled again.
        """

        def double(ant):
            if not ant.is_doubled:
                ant.damage *= 2
                ant.is_doubled = True

        def helper(place):
            if isinstance(place, AntHomeBase):
                pass
            else:
                if place.ant:
                    if isinstance(place.ant, ContainerAnt):
                        ...
                        if place.ant.contained_ant:
                            double(place.ant.contained_ant)
                    else:
                        double(place.ant)
                helper(place.exit)
