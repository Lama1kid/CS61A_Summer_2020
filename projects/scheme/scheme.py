...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        def helper(expr):
            """for being compatible to map() when evaluate list of operands"""
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        evaluated_first_ele = scheme_eval(expressions.first, env)
        evaluated_rest_ele = eval_all(expressions.rest, env)
        return evaluated_first_ele if evaluated_rest_ele is None else evaluated_rest_ele
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        def helper(frame, formals, vals):
            if formals is nil or vals is nil:
                return None
            if formals.rest is nil and vals.rest is nil:
                frame.define(formals.first, vals.first)
            elif formals.rest is not nil and vals.rest is not nil:
                frame.define(formals.first, vals.first)
                helper(frame, formals.rest, vals.rest)
            else:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        def convert_to_python_list(args):
            if args is nil:
                return []
            if args.rest is nil:
                return [args.first]
            else:
                return [args.first] + convert_to_python_list(args.rest)

        python_args = convert_to_python_list(args)
        ...
        try:
            ...
            raise SchemeError(
                f'expect {len(python_args) if self.use_env == False else len(python_args)-1} arguments were passed'
            )
        # END PROBLEM 3
...
class LambdaProcedure(Procedure):
    ...
    def __init__(self, formals, body, env):
        ...
        whose body is the Scheme list BODY(Scheme list of body expressions), and whose parent environment
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        procedure_name = target.first
        formal_paras = target.rest
        procedure_body = expressions.rest
        env.define(procedure_name,
                   do_lambda_form(Pair(formal_paras, procedure_body), env))
...
def do_lambda_form(expressions, env):
    ...
    def helper(body_expressions):
        """
        take as arguments body_expressions
        return constructed scheme list of body_expressions
        """
        if body_expressions.rest is nil:
            return Pair(body_expressions.first, nil)
        else:
            return Pair(body_expressions.first, helper(body_expressions.rest))

    body = helper(expressions.rest)
