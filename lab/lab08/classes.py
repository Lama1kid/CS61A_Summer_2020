...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        player_deck_copy = player.deck.cards[:]
        for card in player_deck_copy:
            card.attack += other_card.attack
            card.defense += other_card.defense
        player.deck.cards = player_deck_copy

        opponent_deck_copy = opponent.deck.cards[:]
        for card in opponent_deck_copy:
