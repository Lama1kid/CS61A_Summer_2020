...
def village(apple, t):
    ...
    def graft(t, bs):
        ...
        """
        if is_leaf(t):
            return tree(label(apple(label(t))), bs)
        new_branches = [branches(apple(label(b))) for b in branches(t)]
        return tree(label(apple(label(t))), [tree(label(b), label(apple(label))branches(t)) for b in branches(t)]) for b in bs])
    base_t = t
    bs = branches(apple(label(t)))
