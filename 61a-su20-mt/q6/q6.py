...
def copycat(lst1, lst2):
    ...
    ['c', 'c', 'c']
    """
    def copycat_helper(lst1, lst2, lst_sofar):
        # I will reduce lst1 and lst2 to simplify the problem in recursion
        if len(lst1) == 0 or len(lst2) == 0:
            # that is the end of recursion
            return lst_sofar
        # focus on the first element
        if lst2[0] >= 0:
            lst_sofar = lst_sofar + [lst1[0] for _ in range(lst2[0])]
        else:
            lst_sofar = lst_sofar[:lst2[0]]
        return copycat_helper(lst1[1:], lst2[2:], lst_sofar) 
